...
class ThorEnv(Controller):
    ...
    def __init__(
        ...
        if headless:
            super().__init__(
                headless=headless, platform="CloudRendering", **controller_kwargs
            )
        else:
            super().__init__(
    ...
    def get_agent_event(self, agent_index=0):
        ...
        return self._unwrap_event(
            getattr(self, "last_event", None), agent_index=agent_index
        )
    ...
    def reset(
        ...
        if isinstance(scene_name_or_num, str):
    ...
    def step(self, action, smooth_nav=False, **kwargs):
        ...
        action_for_state = dict(action)
        if kwargs:
            action_for_state.update(kwargs)
        ...
        event = self.update_states(action_for_state)
        self.check_post_conditions(action_for_state)
        return event
        ...
        if event.metadata["lastActionSuccess"]:
            ...
            if action["action"] == "ToggleObjectOn" and "Faucet" in action["objectId"]:
                ...
                self.cleaned_objects = (
                    self.cleaned_objects | set(cleaned_object_ids)
            ...
            if action["action"] == "CloseObject" and "Fridge" in action["objectId"]:
                ...
                self.cooled_objects = (
                    self.cooled_objects | set(cooled_object_ids)
    ...
    def smooth_look(self, action, render_settings=None, **step_kwargs):
        ...
        end_horizon = start_horizon + constants.AGENT_HORIZON_ADJ * (
            1 - 2 * int(action["action"] == "LookUp")
        )
        position = event.metadata["agent"]["position"]
    ...
    def to_thor_api_exec(self, action, object_id="", smooth_nav=False, agent_id=None):
        ...
        if "RotateLeft" in action:
            ...
            action = with_agent(
                dict(action="PickupObject", objectId=object_id, manualInteract=True)
            )
            event = self.step(action)
            ...
            if (
                len(inventory_objects) == 0
                or "Knife" not in inventory_objects[0]["objectType"]
            ):
            ...
            raise Exception(
                "Invalid action. Conversion to THOR API failed! (action='"
                + str(action)
                + "')"
            )
    ...
    def check_clean(self, object_id):
        ...
        if event.metadata["lastActionSuccess"] and "Faucet" in object_id:
            ...
            for in_sink_obj_id in sink_basin_obj["receptacleObjectIds"]:
                if (
                    game_util.get_object(in_sink_obj_id, event.metadata)["dirtyable"]
    ...
    def get_interactable_poses(self, object_id: str, **kwargs) -> List[Dict]:
        reachable = self.step(action="GetReachablePositions").metadata["actionReturn"]
        poses = self.step(
    ...
    def has_interactable_pose(self, object_id: str) -> bool:
        return bool(self.get_interactable_poses(object_id))

    def get_axis_aligned_bounding_box(
        self, object_id: str, metadata: Optional[Dict] = None
    ):
        if metadata is None:
            metadata = self.step(action="Pass").metadata
        for obj in metadata["objects"]:
            if obj["objectId"] == object_id:
                return obj["axisAlignedBoundingBox"]

    def get_oriented_bounding_box(self, object_id: str):
        metadata = self.step(action="Pass").metadata
        for obj in metadata["objects"]:
            if obj["objectId"] == object_id:
                return obj["objectOrientedBoundingBox"]

    def get_top_down_frame(self) -> "Image.Image":
        # Setup the top-down camera
        event = self.step(action="GetMapViewCameraProperties", raise_for_failure=True)
